...
The composer model menu and the Settings → Models catalog list only vendors that have a key in GeoCRM Settings (ChatGPT, Gemini, Claude, Grok, DeepSeek, and the rest of the allowlist, in that order). A vendor with no key, including DeepSeek, does not appear until you add the key there. Settings → Models does not add a second LLM provider. The [model configuration guide](./providers.md) covers the Web UI DeepSeek card and custom OpenAI-compatible endpoints.
