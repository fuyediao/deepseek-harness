...
Start the GeoCRM desktop app through the [root README](../../../README.md#run) with `dsh electron` (or `dsh --profile electron`), or by opening the installed Windows app. This guide begins after the window is open. The Host uses its invoking directory as the default filesystem location, but a fresh window has no selected workspace until you add one. The installed exe starts that Host itself; `dsh electron` remains the source-tree launcher.
