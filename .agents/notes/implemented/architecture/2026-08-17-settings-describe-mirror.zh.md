...
本决策更新了[通过 Host settings 持久化 Web 用户偏好](../bug-fix/2026-08-06-host-backed-web-preferences.zh.md)和[由插件自己拥有的设置表层](2026-08-12-plugin-owned-settings-surface.zh.md)所记录的浏览器读取与失效机制，同时保留其中关于偏好所有权与命名空间暴露的决策。它也取代了[首次运行就绪状态](../bug-fix/2026-08-12-onboarding-reads-every-provider.zh.md)所记录的 Models 页联接直读描述；该联接的 settings 部分现在从本镜像派生。
