...
- The desktop patch mounts `file-upload` with `connection`. `session-controller` injects `fileUploads`; without that row the Host prints `ipc ready` and then exits with `waiting for service: fileUploads`.
- The desktop patch pins `@deepseek-ai/dsh-host-directory-picker-native` because `directory-picker-auto` injects `webServer`. That host row does not occupy `conversation.hero.workspace.directoryFlow`. The matching client surface `@deepseek-ai/dsh-client-ui-directory-picker-native` is a roster row so the Choose workspace chip can open the OS folder dialog when the list is empty.
