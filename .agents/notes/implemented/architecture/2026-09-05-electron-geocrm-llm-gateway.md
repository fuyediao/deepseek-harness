...
The desktop product name is GeoCRM Harness: native window title, NSIS `productName`, sidebar brand occupants (priority `-10`, so they shadow the official DeepSeek mark), sign-in cover, desktop welcome notice, `dsh electron` help text, and the `app:electron-surface` prompt. Package names, the `dsh` CLI verb, and `dsh web` keep DeepSeek Harness.
...
- `apps/electron/tests/window-chrome.spec.ts` rewrites the official frontend title suffix to GeoCRM Harness. `apps/electron/tests/google-sign-in.spec.ts` covers the loopback authorize URL, CSRF state, and token POST.
