...
`packages/client/ui-settings-models` maps `llm-geocrm` to a curated editor: a session bar (Sign out returns to the window cover), the shared model-list editor so Fetch can call registered discovery, and Customized settings for origin plus a write-only token paste fallback. On the desktop renderer the Models page uses catalog copy, hides Add provider / Add a custom provider, and opens the GeoCRM card so the allowlist is visible. The composer and `/model` popup split `geocrm` composite ids into vendor groups in GeoCRM Electron order (ChatGPT, Gemini, Claude, Grok, then the rest) and show `Vendor · model` on the trigger; selection still stores `provider: geocrm`. `listModels` joins `GET /ai/models` with `POST /ai/settings/connectivity`: vendors that appear have a BYOK key (probe outcome is ignored); omitted vendors carry `geocrm:not-configured`. The composer shows Not Configured and refuses a new pick. A failed or unrecognized connectivity body leaves rows unmarked. A catalog `configured: false` flag is also honored. Employee-id or email sign-in (public `POST /auth/password` and `POST /auth/public/resolve-employee-id`) and desktop Google sign-in (`GET /auth/google` in the system browser, tokens returned on a short-lived `127.0.0.1` loopback) live on the window cover. After cover sign-in the cover probes `POST /ai/harness/tools/list_my_access` and keeps the window locked unless `desktop_agent` is granted. `dsh web` does not mount this card.
...
- `packages/llm/llm-geocrm/tests` cover catalog ids, HTTP mapping, Responses translation, adapter fetch, plugin `apply`, session refresh, and BYOK presence from connectivity.
...
**Add `GET /ai/keys` to GeoCRM for a boolean map.** Deferred: `POST /ai/settings/connectivity` already omits vendors with no key. A dedicated endpoint would avoid vendor probes when opening the picker.

## Consequences

- The desktop window shows a GeoCRM Harness sign-in panel before the conversation shell. Continue with Google opens the system browser; employee ID / email stay on that panel. The taskbar, title bar, sidebar, and blank-session hero show GeoCRM Harness. Settings → Models still shows GeoCRM, not DeepSeek: catalog copy, the opened GeoCRM card, Sign out, and a token-paste fallback under Customized. It does not offer Add provider. The composer lists vendor groups from the GeoCRM allowlist and marks a vendor with no GeoCRM key as Not Configured. Vendor keys stay in GeoCRM Settings; the user needs `desktop_agent`.
