// Keyless browser e2e: the versioned notice is the only first-run dialog, the
// shipped DeepSeek adapter stays mounted while its credential is absent, and
// the Models-page key write lands in an isolated harness home without a reload
// or model call.
...
describe.skipIf(MODE === 'record')('web e2e: first-run welcome notice and Models key write', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    ...
    // and no credential dialog follows it.
  ...
  it('never paints the takeover chrome on a configured reload, even with the settings join held open', async () => {
    ...
    // Regression pin for the reload flash: the welcome step is already
    // acknowledged, yet it must load that fact before deciding not to show.
    // Dialog chrome lives inside the visible branch, so the deciding window
    // paints and blocks nothing. Holding settings/describe widens that window
    // from loopback RTT scale to a deterministic hundreds of milliseconds,
    // removing all timing dependence from the sampler assertions below.
    //
  ...
  it('keeps the fixture inventory closed', async () => {
    await assertFixtureInventory(
      SNAPSHOT_DIR,
      ['welcome.expected.md', 'models.expected.md', 'default-models.expected.md'],
    )
  })
})
