...
Examples:
  ...
  dsh --profile electron                     boot the electron profile (same as: dsh electron)
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  const electron = program.command('electron').description('boot the electron profile (alias of --profile electron); the desktop app\'s own flags follow')
  electron
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the desktop app (see: dsh electron --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
