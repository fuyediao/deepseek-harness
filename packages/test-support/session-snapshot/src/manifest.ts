...
export type SnapshotProfile = 'headless' | 'sdk' | 'acp' | 'web' | 'electron'
...
const PROFILES = new Set<SnapshotProfile>(['headless', 'sdk', 'acp', 'web', 'electron'])
...
export function parseSnapshotManifest(source: string, path = 'snapshot.yml'): SnapshotManifest {
  ...
  try {
    ...
    if (typeof root.profile !== 'string' || !PROFILES.has(root.profile as SnapshotProfile)) {
      throw new Error('manifest.profile must be headless, sdk, acp, web, or electron')
    }
