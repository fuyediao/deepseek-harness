---
description: "共享工具家族的包映射：原子文件写入、品牌化 id、双端队列、Electron IPC 帧协议、JSON 值、harness 主目录路径、启动环境、出站代理策略、原生命令、输出保留、时区与超时。"
kind: "package-group"
---
...
`util/` 组为能力包提供共享的机制原语，避免重复实现。它涵盖原子写入、品牌化 id、双端队列、Electron 桌面壳的 IPC 帧协议、无损 JSON 值、UUID、Harness home 路径、启动环境、出站代理策略、原生命令、输出保留、时区规范化和超时处理。这里的每个根入口都是库：它不注册产品服务或事件，业务语义仍由消费它的能力负责。
...
| [`electron-ipc/`](electron-ipc/README.zh.md) | Electron 桌面壳 Host 与主进程之间按行分隔的 JSON 帧协议 |
