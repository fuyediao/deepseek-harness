...
模型按提供方分组。`geocrm` 路由上的复合 id 会拆成供应商分组（ChatGPT、Gemini、Claude、Grok，然后是其余），触发器显示 `供应商 · 模型`；选中后仍存储 `provider: geocrm`。菜单只显示模型与推理强度名称；目录中的说明仍可供其他消费方使用。`/model` 弹窗应用所选模型的默认推理强度；composer 随后可以选择任一已公布的推理强度。适配器没有推理元数据时不显示 Effort 行；不存在任意推理强度输入。
