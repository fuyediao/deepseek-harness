...
模型按提供方分组。`geocrm` 路由上的复合 id 会拆成供应商分组（OpenAI、Google、Anthropic、xAI，然后是其余），触发器显示 `供应商 · 模型`；选中后仍存储 `provider: geocrm`。composer 菜单只显示模型与推理强度名称。没有密钥的 GeoCRM 供应商不会进入 Host 目录，因此这里的菜单也不显示。残留的当前未配置模型在触发器上标为未设定，且不可选择。`/model` 弹窗显示提供方名称与目录说明；其中两个内置 DeepSeek 模型的说明使用当前语言，外部提供方说明保持原文。弹窗应用所选模型的默认推理强度；composer 随后可以选择任一已公布的推理强度。适配器没有推理元数据时不显示 Effort 行；不存在任意推理强度输入。
