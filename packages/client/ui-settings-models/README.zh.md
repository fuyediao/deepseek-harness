---
description: "dsh Web 客户端的模型设置与产品引导插件：提供方行、API 密钥管理、模型列表与版本化首次运行声明。"
...
`dsh-client-ui-settings-models` 是 dsh Web 客户端的 Models 设置页面：用户可以配置 API 密钥（以只写方式存入 profile 的凭据引用之下）、编辑每个提供方的模型列表，并手工声明自定义 pi-ai 路由；页面以提供方行展示，一次只展开一张编辑卡片。该页面把提供方目录、设置文档与凭据描述合并为一个共享快照，因此行的状态在三个方面始终一致。它还会向首次运行的用户展示版本化内测声明。
...
版本化声明是本包在 `settings.onboarding` 中注册的唯一步骤。API 密钥在本页输入。用户已经能够到达的**任何**提供方都会结束首次运行设置卡片姿态；没有任何提供方的用户会看到官方 DeepSeek 卡片展开，直到关闭它或存入密钥。

### 桌面登录

在 Electron 渲染进程中，本包还会占据 `shell.gate`、侧栏品牌席位以及空白会话首屏标志与名称：窗口停留在居中的 GeoCRM Harness 登录卡片，直到存入会话令牌，标题栏、侧栏和新会话首屏显示 GeoCRM 地图针与 GeoCRM Harness。侧栏、登录页与首屏标志都是无底的透明地图针。登录页提供「使用 Google 登录」（系统浏览器，回环返回）以及工号或邮箱。设置 → 模型 列出 GeoCRM 目录与退出登录，不重复登录表单、API 密钥导语或添加提供方。模型卡片列出全部 GeoCRM 模型；没有密钥的供应商显示未设定，且不能开启。composer 只列出已开启且供应商已有密钥的模型，包括 DeepSeek。`dsh web` 把这些席位留给官方品牌，并隐藏 Google 按钮。
...
声明步骤在 `src/client/locales.ts` 中持有精确文案，并在 `src/onboarding-copy.ts` 中持有确认版本；回环时它通过既有 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`，且只有显式点击「继续」才会记录当前版本。非回环浏览器无法使用这个仅限宿主的 namespace，因此确认只保留在进程内，刷新后声明会再次出现。
