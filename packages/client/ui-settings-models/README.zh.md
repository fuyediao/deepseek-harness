...
在 Electron 渲染进程中，本包还会占据 `shell.gate`、侧栏品牌席位以及空白会话首屏标志与名称：窗口停留在居中的 GeoCRM Harness 登录卡片，直到存入会话令牌，标题栏、侧栏和新会话首屏显示 GeoCRM 地图针与 GeoCRM Harness。侧栏、登录页与首屏标志都是无底的透明地图针。登录页提供「使用 Google 登录」（系统浏览器，回环返回）以及工号或邮箱。设置 → 模型 列出 GeoCRM 目录与退出登录，不重复登录表单、API 密钥导语或添加提供方。`dsh web` 把这些席位留给官方品牌，并隐藏 Google 按钮。
