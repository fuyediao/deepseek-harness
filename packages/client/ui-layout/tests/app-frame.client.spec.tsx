...
function mountFrame(windowWidth = frameWidth, options: { gate?: ReactNode } = {}) {
  vi.stubGlobal('innerWidth', windowWidth)
  const instance = createLayoutStore().create()
  const slotCalls: { key: string; props: object; options: RenderOpts | undefined }[] = []
  const renderSlot: AppFrameProps['renderSlot'] = (key, owner, opts) => {
    slotCalls.push({ key, props: owner, options: opts })
    if (key === 'shell.gate' && options.gate !== undefined) return options.gate
    return <div data-testid={`${key}-content`} data-entry-key={opts?.entryKey} />
  }
...
describe('AppFrame', () => {
  ...
  it('paints a blocking occupant in the gate layer above the overlay', () => {
    const { frame, getByTestId } = mountFrame(frameWidth, {
